...
export function RecentPosts() {
  return <Card className="bg-[#1A1F2C]/50 border-purple-500/20">
      <div className="p-4 border-b border-purple-500/20 flex justify-between items-center">
        <h2 className="text-xl text-slate-50 font-bold">Recent Posts</h2>
        <div className="flex gap-2">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
            <Input type="search" placeholder="Search posts..." className="pl-10 bg-[#1A1F2C]/50 border-purple-500/20" />
          </div>
      ...
      <Table>
        ...
        <TableBody>
          {recentPosts.map(post => <TableRow key={post.id}>
              <TableCell className="font-medium">{post.title}</TableCell>
              <TableCell>
                <span className={`px-2 py-1 rounded-full text-xs ${post.status === 'published' ? 'bg-green-500/20 text-green-400' : post.status === 'draft' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}>
                  {post.status}
                </span>
              </TableCell>
              ...
              <TableCell>
                {post.trend === 'up' ? <ArrowUp className="h-4 w-4 text-green-400" /> : post.trend === 'down' ? <ArrowDown className="h-4 w-4 text-red-400" /> : <span className="text-gray-400">-</span>}
              </TableCell>
              <TableCell>
                <div className="flex gap-2">
