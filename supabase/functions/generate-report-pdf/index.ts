  ...
  try {
    ...
    const { error: contactError } = await supabase
      ...
      .insert({
        ...
        email,
        phone,
        report_sent: true, // Set to true as they're downloading it directly
      });
    ...
    console.log(`[${new Date().toISOString()}] ✅ Contact saved successfully`);
    ...
    console.log(`[${new Date().toISOString()}] 📄 PDF generated successfully with brand colors`);

    return new Response(
      JSON.stringify({
        success: true,
        pdfBase64,
        fileName: `Raport-AI-${name.replace(/\s+/g, '-')}-${Date.now()}.pdf`,
      }),
      {
    ...
    console.error(`[${new Date().toISOString()}] ❌ Error in generate-report-pdf function:`, {
